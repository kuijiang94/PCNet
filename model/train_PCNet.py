...
file1 = 'PCNet_PAR.txt'
file2 = 'PCNet_PSNR.txt'
file3 = 'PCNet_LOSS.txt'
######### Set Seeds ###########
random.seed(1234)
np.random.seed(1234)
...
############### me
scheduler = optim.lr_scheduler.StepLR(step_size=60, gamma=0.8, optimizer=optimizer)####step_size epoch, best_epoch 445
scheduler.step()
...
#train_loader = DataLoader(dataset=train_dataset, batch_size=opt.OPTIM.BATCH_SIZE, shuffle=True, drop_last=False, pin_memory=True) ### train on windows
train_loader = DataLoader(dataset=train_dataset, batch_size=opt.OPTIM.BATCH_SIZE, shuffle=True, num_workers=16, drop_last=False, pin_memory=True)  ### train on Linux

val_dataset = get_validation_data(val_dir, {'patch_size':opt.TRAINING.VAL_PS})
#val_loader = DataLoader(dataset=val_dataset, batch_size=16, shuffle=False, drop_last=False, pin_memory=True) ### test on windows
val_loader = DataLoader(dataset=val_dataset, batch_size=16, shuffle=False, num_workers=8, drop_last=False, pin_memory=True)  ### test on Linux
...
for epoch in range(start_epoch, opt.OPTIM.NUM_EPOCHS + 1):
    ...
    for i, data in enumerate(tqdm(train_loader), 0):
        ...
        loss = 0.3*(loss_char0 + 0.2*loss_char1) + (0.2*(loss_edge0)) - (0.15*(loss_SSIM0 + 0.2*loss_SSIM1))# 0.05,0.2
        loss.backward()
        optimizer.step()
